...
if states=="Andhra Pradesh":
    ...
    if fare=='50-1000':
        mydb = mysql.connector.connect(host="localhost",user="root",port="3306",database="pro1",password="181506")
        mycursor = mydb.cursor()
        qurey=f'''select * from busdetails
                    where Price Between 50 and 1000 and Route_name="{A}"
                    order by Price desc'''
        mycursor.execute(qurey)
        out=mycursor.fetchall()
        df=pd.DataFrame(out,columns=["ID","Route_name","Route_link","Bus_name","Bus_type","Starting_time","Duration","Reaching_time","Star_rating","Price","Seats_available"])
        st.write(df)
    if fare=='1000-2000':
        mydb = mysql.connector.connect(host="localhost",user="root",port="3306",database="pro1",password="181506")
        mycursor = mydb.cursor()
